...
class SpecialVortragende extends SpecialPage {
	function __construct() {
		parent::__construct( 'Vortragende' );
	}
	...
	function execute( $par ) {
		...
		foreach( $members as $member ) {
			...
			if( $this->updated( $vorlage ) ) {
				...
				// TODO: change back this temporary hack!
				if( strcmp( $title, "Der Satz von Church" ) < 0 ) {
					$vorlageNeu = preg_replace( "/\|update=false/Us", "", $vorlage );
					}
